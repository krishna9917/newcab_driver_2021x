...
import com.softechurecabdriver.provider.data.network.model.Request_;
    ...
    public View initView(View view) {
        ...
        if (DATUM != null) {
            ...
            if(datum.getPaymentMode().equals("CASH")){
                ...
                paymentModeImg.setBackground(ContextCompat.getDrawable(requireContext(), R.drawable.ic_debit_machine));
            }else if(datum.getPaymentMode().equals("ONLINE")) {
                lblPaymentType.setText("ONLINE");
                paymentModeImg.setBackground(ContextCompat.getDrawable(requireContext(), R.drawable.ic__online));
            }else{
                lblPaymentType.setText("CARD");
                paymentModeImg.setBackground(ContextCompat.getDrawable(requireContext(), R.drawable.ic_card));
            }
            ...
            if (datum.getPayment() != null) {
                if (datum.getPayment().getPayable() > 0) {
                    ...
                    if(datum.getPaymentMode().equals("CARD")){
                        payableAmount.setText(Constants.Currency + Double.parseDouble(Math.round(datum.getPayment().getProviderPay()) + ""));
                    }else{
                        payableAmount.setText(Constants.Currency + Double.parseDouble(Math.round(datum.getPayment().getPayable()) + ""));
                    }
                } else llAmountToBePaid.setVisibility(View.GONE);
            }else{
                Toasty.info(this.getContext(), "Failed to display data! Please minimize the provider and open again.", Toast.LENGTH_LONG).show();
