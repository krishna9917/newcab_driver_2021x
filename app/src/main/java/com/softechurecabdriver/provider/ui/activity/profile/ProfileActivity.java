...
import com.softechurecabdriver.provider.ui.activity.change_password.ChangePasswordActivtiy;
...
//            fbOtpVerify(countryCode, countryDialCode, "");
    ...
    public void onSuccess(UserResponse user) {
        ...
        if(user.getGender()=="Male") sexy_Spinner.setSelection(0);
        else if(user.getGender()=="Female") sexy_Spinner.setSelection(1);
        sexy_Spinner.setEnabled(false);
        ...
        Glide.with(activity()).load(BuildConfig.BASE_IMAGE_URL + user.getAvatar())
                         .apply(RequestOptions
