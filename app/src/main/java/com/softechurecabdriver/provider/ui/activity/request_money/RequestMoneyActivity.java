...
import com.softechurecabdriver.provider.data.network.model.RequestedDataItem;
import com.softechurecabdriver.provider.ui.adapter.RequestAmtAdapter;
...
public class RequestMoneyActivity extends BaseActivity implements RequestMoneyIView {
    ...
    public void onViewClicked() {
        if (TextUtils.isEmpty(etRequestAmt.getText())) {
        } else if (Double.parseDouble(etRequestAmt.getText().toString()) <= walletAmt && walletAmt >= 50.0)
            ...
            Toast.makeText(this, "Saldo mínimo insuficiente!", Toast.LENGTH_SHORT).show();
            etRequestAmt.setText(null);
