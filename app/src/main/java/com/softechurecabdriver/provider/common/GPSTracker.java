...
import com.softechurecabdriver.provider.common.kalman.KalmanLocationManager;
            ...
            if (location.getProvider().equals(KalmanLocationManager.KALMAN_PROVIDER)) {
                ...
                try {
                    if (Utilities.isConnected()) {
                        ...
                        if (SharedHelper.getKey(getApplicationContext(), Constants.SharedPref.LOGGGED_IN).equalsIgnoreCase("true")) {
                            HashMap<String, Object> params = new HashMap<>();
                            params.put("latitude", mLastKnownLocation.getLatitude());
                            params.put("longitude", mLastKnownLocation.getLongitude());
                            //presenter.getLocationUpdate(params);
                        } else {
                            return;
                        }
                    }
                ...
                if (DATUM != null && DATUM.getStatus().equalsIgnoreCase("ACCEPTED") || DATUM != null && DATUM.getStatus().equalsIgnoreCase("STARTED") || DATUM != null && DATUM.getStatus().equalsIgnoreCase("ARRIVED") || DATUM != null && DATUM.getStatus().equalsIgnoreCase("PICKEDUP")) {
         ...
         //   System.out.println("RRR GPSTracker.saveLocationToFireBaseDB :: " + refPath);
            if (!refPath.equals("loc_p_0"))
                try {
                    ...
                    mLocationRef.setValue(new LatLngFireBaseDB(endLat, endLng, bearing));
                } catch (Exception e) {
                    e.printStackTrace();
                }
        }
